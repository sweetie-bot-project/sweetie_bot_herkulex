...
protected:
	...
	std::map<std::string, std::shared_ptr<herkulex::servo::RegisterValues>>
	    servos_init;                                                 /**< Registers initializeted on startup. */
...
protected:
	...
	bool ackPosVel(const HerkulexPacket& ack, const std::string& servo, double& pos, double& vel,
	               servo::Status& status);
	bool reqState(HerkulexPacket& req, const std::string& servo);
	bool ackState(const HerkulexPacket& ack, const std::string& servo, HerkulexServoState& state,
	              servo::Status& status);
