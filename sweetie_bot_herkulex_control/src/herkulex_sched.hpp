...
protected:
	...
	public:
		...
		void timeout(TimerId id) { owner->trigger(); }
	};
	enum SchedTimers
	{
...
protected:
	...
	RTT::OperationCaller<bool(const HerkulexPacket& ack, const std::string& servo, double& pos, double& vel,
	                          servo::Status& status)>
	    ackPosVel;
	RTT::OperationCaller<bool(HerkulexPacket& req, const std::string& servo)> reqState;
	RTT::OperationCaller<bool(const HerkulexPacket& ack, const std::string& servo, HerkulexServoState& state,
	                          servo::Status& status)>
	    ackState;
