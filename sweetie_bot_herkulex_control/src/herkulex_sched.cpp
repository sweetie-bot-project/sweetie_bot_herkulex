      ...
      ackPosVel("ackPosVel"), reqState("reqState"), ackState("ackState")
{
	...
	this->addProperty("period_CM", period_CM)
	    .doc(
	        "Duration of configuration and monitoring exchange round. During this round the component forwards request "
...
void HerkulexSched::updateHook()
{
		...
		if (sync_port.read(timer_id) == NewData)
		{
			...
			else
			{
				...
				if (log(DEBUG))
				{
					...
					log() << "REQ packet: servo_id: " << int(req_pkt.servo_id) << " cmd: " << int(req_pkt.command)
					      << " data(" << req_pkt.data.size() << "): ";
	...
	case RECEIVE_READ_ACK:
		...
		while (!ack_buffer.empty())
		{
			...
			{
				log() << std::dec << std::setw(2) << std::setfill('0');
				log() << "ACK packet: servo_id: " << int(ack_pkt->servo_id) << " cmd: " << int(ack_pkt->command)
				      << " data(" << ack_pkt->data.size() << "): ";
			...
			if (success)
			{
				...
				// check sendPacketDL result: possible deadlock detection
				{
					SendStatus result = sendPacketDL_handle.collectIfDone();
					if (result != SendSuccess)
						log(WARN) << "sendPacketDL() operation failed (ACK received). SendStatus: " << result
						          << endlog();
				}
