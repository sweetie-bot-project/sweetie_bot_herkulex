...
HerkulexArray::HerkulexArray(std::string const& name)
    : TaskContext(name, PreOperational), log(logger::categoryFromComponentName(name)),
      ...
      timeout_timer(this), break_loop_flag(false), sendPacketCM("sendPacketCM")
{
	if (!log.ready())
	{
	...
	this->addProperty("servos", servos_prop)
	    .doc("Servo descriptions (PropertuBag). Format: \n"
	         "\t\t\t{\n"
	         "\t\t\t    PropertyBag servo_name1 { string servo_id, string servo_model, uint offset, double scale, bool "
	         "reverse, PropertyBag register_init { uint register1, uint register2, ... } },\n"
	         "\t\t\t    PropertyBag servo_name2 { ... }\n"
	         "\t\t\t    ...\n"
	         "\t\t\t}");
	this->addProperty("tryouts", tryouts_prop).doc("Number of attemts to perform one operation.").set(3);
	this->addProperty("timeout", timeout_prop).doc("Operation timeout (seconds).").set(0.1);
	this->addProperty("mass_reset", mass_reset_prop)
	    .doc("Reset all servos with broadcast command. This is faster then per servo reset but some servos can be "
	         "turned "
	         "off for too long time.")
	    .set(true);
	...
	this->provides("protocol")
	    ...
	    .doc("Generate READ packet for position and velocity query, cause exeception on failure, return false on "
	         "invalid "
	         "packet.")
	...
	this->provides("protocol")
	    ...
	    .doc("Decode extended state query (position, velocite, pwm, target, desired position and speed), cause "
	         "exeception on failure, return false on invalid packet.")
	    .arg("ack", "Reference to received packet (HerkulexPacket).")
	    .arg("servo", "Servo name.")
	    .arg("state",
	         "Reference to HerkulexServoState msg, if query has succed extended servo state will be pushed back.")
	    .arg("status", "Servo status (byte0=status_error, byte1=status_detail).");
}
...
bool HerkulexArray::configureHook()
{
	...
	for (PropertyBag::const_iterator s = servos_prop.begin(); s != servos_prop.end(); s++)
	{
		...
		if (scale_prop.ready())
			scale = scale_prop.rvalue();

		/*if (servo_model_prop.rvalue() == "drs0101" || servo_model_prop.rvalue() == "drs0201") {
		        servos.addServo(new HerkulexServoDRS101(name, servo_id_prop.rvalue(), reverse_prop.rvalue(),
	offset_prop.rvalue()));
		}
		else {
	log(ERROR) << "Incorrect servos structure: unknown servo model: " << servo_model_prop.rvalue() << ". Known models:
	drs101, drs202." << endlog(); return false;
		...
		{
			for (PropertyBag::const_iterator p = registers_init_prop.rvalue().begin();
			     p != registers_init_prop.rvalue().end(); p++)
			{
				Property<unsigned int> reg_val_prop(*p);
				if (!reg_val_prop.ready() || !servo->register_mapper.findByName(reg_val_prop.getName()))
				{
					log(ERROR) << "Incorrect servos structure: registers_init contains invalid property: " << servo_name
					           << "." << reg_val_prop.getName() << endlog();
					return false;
				}
				if (reg_init->find(reg_val_prop.getName()) != reg_init->end())
				{
					log(WARN) << "Servos structure: registers_init contains dublicate properties: " << servo_name << "."
					          << reg_val_prop.getName() << endlog();
				}
				reg_init->insert(servo::RegisterValues::value_type(reg_val_prop.getName(), reg_val_prop.rvalue()));
				log(DEBUG) << "Cachce reg = " << reg_val_prop.getName() << " val = " << std::dec << std::setw(2)
				           << std::setfill('0') << reg_val_prop.rvalue() << resetfmt << endlog();
			}
		}
	}
	...
	if (bcast == servos.end())
	{
		log(ERROR) << "Incorrect servos structure: current version of HerkulexArray requeres brodcast servo record."
		           << endlog();
		return false;
	}
...
bool HerkulexArray::setGoal(const std::string& servo, unsigned char mode, double goal, double playtime)
{
	try
	{
		...
		if (mode & servo::JOGMode::SPEED_CONTROL)
			goal_raw = s.convertVelRadToRaw(goal);
		else
			goal_raw = s.convertPosRadToRaw(goal);
	...
	try
	{
		...
		for (servo::HerkulexServoArray::const_iterator iter = servos.begin(); iter != servos.end(); iter++)
		{
			...
			if (sendRequest(req_pkt, s->ackCallbackStat(status)))
			{
				...
				{
					log(WARN) << s->getName() << " ID = " << std::dec << s->getID() << std::dec
...
std::string HerkulexArray::statusToString(servo::Status status)
{
	...
	if (status.detail & servo::Status::MOTOR_ON)
		status_str << "ON ";
	else
		status_str << "OFF ";
	if (status.detail & servo::Status::MOVING)
		status_str << "moving ";
	if (status.detail & servo::Status::INPOSITION)
		status_str << "inpos ";
	if (status.error & servo::Status::ERROR_MASK)
	{
		status_str << "ERR ( ";
		if (status.error & servo::Status::ERROR_OVER_VOLTAGE)
			status_str << "voltage ";
		if (status.error & servo::Status::ERROR_POT_LIMIT)
			status_str << "pot_limit ";
		if (status.error & servo::Status::ERROR_TEMPERATURE)
			status_str << "temperature ";
		if (status.error & servo::Status::ERROR_OVERLOAD)
			status_str << "overload ";
		if (status.error & servo::Status::ERROR_DRIVER_FAULT)
			status_str << "driver_fault ";
		if (status.error & servo::Status::ERROR_EEP_REGS)
			status_str << "eep_regs ";
		status_str << ") ";
	}
	else
	...
	if (status.error & servo::Status::INVALID_PACKET)
	{
		...
		if (status.detail & servo::Status::INVALID_PACKET_CHECKSUM)
			status_str << "checksum ";
		if (status.detail & servo::Status::INVALID_PACKET_UNKNOWN_CMD)
			status_str << "cmd ";
		if (status.detail & servo::Status::INVALID_PACKET_REG_RANGE)
			status_str << "reg_range ";
		if (status.detail & servo::Status::INVALID_PACKET_FRAME_ERROR)
			status_str << "frame_err ";
		status_str << ") ";
	};
...
void HerkulexArray::printServoStatus(const std::string& servo)
{
	try
	{
		...
		if (success)
		{
			std::cout << servo << " ID = " << std::dec << s.getID() << std::dec << " \t" << statusToString(status)
			          << std::endl;
		}
		else
		{
...
void HerkulexArray::printErrorServoStatuses()
{
	...
	for (servo::HerkulexServoArray::const_iterator iter = servos.begin(); iter != servos.end(); iter++)
	{
		...
		if (success)
		{
			if (status.error & servo::Status::ERROR_MASK)
			{
				std::cout << s.getName() << " ID = " << std::dec << s.getID() << " " << std::dec
...
void HerkulexArray::discoverServos()
{
	for (unsigned int id = 0; id < 0xfe; id++)
	{
		...
		if (std::any_of(servos.cbegin(), servos.cend(),
		                [&id](const servo::HerkulexServoArray::value_type& kv) { return kv.second->getID() == id; }))
		{
			continue;
		}
		// Create temporary object to access servo.
		std::shared_ptr<servo::HerkulexServo> servo(
		    new servo::HerkulexServoDRS101("servo_id_" + std::to_string(id), id));
		// Request servo.
		servo::Status status;
		...
		if (!addServo(servo))
		{
			...
			std::cout << "But servo with name '" << servo->getID() << "' is already present in array. Skipping."
			          << std::endl
			          << std::endl;
			continue;
		}
		...
		if (success)
		{
			std::cout << std::dec << "SERVO WITH ID = " << std::dec << id << " (model: " << model[0] << model[1]
			          << ", firmware version: " << version[0] << version[1] << ") IS FOUND." << std::endl;
			std::cout << "Servo is added to array with name '" << servo->getName() << "'." << std::dec << std::endl;
			std::cout << servo->getName() << " ID = " << std::dec << servo->getID() << std::dec << " \t"
			          << statusToString(status) << std::endl;
			std::cout << std::endl;
		}
		else
...
bool HerkulexArray::reqIJOG(HerkulexPacket& req, const ServoGoal& goal)
{
	...
	for (unsigned long i = 0; i < goal.name.size(); i++)
	{
		...
		if (s != servos.end())
		{
			s->second->insertIJOGdata(req, servo::JOGMode::POSITION_CONTROL,
			                          s->second->convertPosRadToRaw(goal.target_pos[i]),
...
bool HerkulexArray::ackPosVel(const HerkulexPacket& ack, const std::string& servo, double& pos, double& vel,
                              servo::Status& _status)
{
	if (!this->isConfigured())
		return false;
	servo::Status status;
...
bool HerkulexArray::ackState(const HerkulexPacket& ack, const std::string& servo, HerkulexServoState& state_array,
                             servo::Status& _status)
{
	if (!this->isConfigured())
		return false;
...
bool HerkulexArray::sendRequest(const HerkulexPacket& req, servo::HerkulexServo::AckCallback ack_callback)
{
	...
	do
	{
		...
		do
		{
			{
				MutexLock lock(ack_mutex);
				while (ack_buffer.empty() && timeout_timer.isArmed(TIMEOUT_TIMER_ID) && !break_loop_flag)
					ack_cond.wait(ack_mutex);
			}
			if (!ack_buffer.empty())
			{
		...
		if (log(DEBUG))
		{
			if (!success)
			{
				if (!break_loop_flag)
					...
					log(DEBUG) << "ACK break loop" << endlog();
			}
		}
