      ...
      timeout_timer(this), break_loop_flag(false), log(logger::categoryFromComponentName(name))
{
  ...
  this->addProperty("mass_reset", mass_reset_prop)
      .doc("Reset all servos with broadcast command. This is faster then per servo reset but some servos can be turned "
...
bool HerkulexArray::resetAllServos()
{
  try
  {
    ...
    for (servo::HerkulexServoArray::const_iterator iter = servos.begin(); iter != servos.end(); iter++)
    {
      ...
      if (!setServoRegisters(s, broadcast_init.get()) || !setServoRegisters(s, servos_init.at(s->getName()).get()))
      {
      ...
      if (sendRequest(req_pkt, s->ackCallbackStat(status)))
      {
        ...
        {
          log(WARN) << s->getName() << " ID = " << std::dec << s->getID() << std::dec << statusToString(status)
                    << endlog();
        }
      }
      else
...
void HerkulexArray::printErrorServoStatuses()
{
  ...
  for (servo::HerkulexServoArray::const_iterator iter = servos.begin(); iter != servos.end(); iter++)
  {
    ...
    if (success)
    {
      if (status.error & servo::Status::ERROR_MASK)
      {
        std::cout << s.getName() << " ID = " << std::dec << s.getID() << " " << std::dec << statusToString(status)
                  << std::endl;
      }
    }
    else
...
void HerkulexArray::discoverServos()
{
  for (unsigned int id = 0; id < 0xfe; id++)
  {
    ...
    if (!addServo(servo))
    {
      ...
      std::cout << "But servo with name '" << servo->getID() << "' is already present in array. Skipping." << std::endl
                << std::endl;
      continue;
    }
    ...
    success = resetServo(servo->getName()) && (model[0] = getRegisterEEP(servo->getName(), "model1")) != READ_ERROR &&
