...
static const double herkulex_servo::HerkulexServoDRS101::POS_CONV_COEFF_RAW2RAD = 0.325*M_PI/180.0;

static const double herkulex_servo::HerkulexServoDRS101::VEL_CONV_COEFF_RAM2RADS = 29.9;

static const double herkulex_servo::HerkulexServoDRS101::VEL_CONV_COEFF_RAM2SEC = 0.0115;

static const std::vector<Register> herkulex_servo::HerkulexServoDRS101::registers =
{
...
{ 6,	"reserved1",                  5,  -1,   1,   true,  "Reserved" },
...
{ 11,	"reserved2",                  10,  4,   1,   true,  "Reserved" },
...
{ 45,	"reserved3",                  -1, 50,   1,   true,  "Reserved" },
{ 46,	"reserved4",                  -1, 51,   1,   true,  "Reserved" },
{ 47,	"torque_contorl",             -1, 52,   1,   true,  "Torque control" },
{ 48,	"led_contorl",                -1, 53,   1,   true,  "Led contorl" },
...
{ 57,	"reserved5",                  -1, 66,   2,  false,  "Reserved" },
...
static const std::vector<Register> herkulex_servo::HerkulexServoDRS101::register_mapper_drs101 = RegisterMapper(registers);
...
{
	return VEL_CONV_COEFF_RAW2RAD * raw;
};
...
{
	return TIME_CONV_COEFF_RAW2S * raw;
};

virtual unsigned int convertTimeSecToRaw(double time) const
{
	return time / TIME_CONV_COEFF_RAW2S;
};
...
virtual void reqPosVel(HerkulexPacket& req) const
{
	...
	req.servo_id = hw_id;
	req.data.resize(2);
	req.date[0] = 58; // RAW addr of Absolute Position
	req.date[1] = 4;
}

virtual bool ackPosVel(HerkulexPacket& ack, double& pos, double& vel, Status& status) const 
{
	...
	if (ack.data[0] != 58 || data[1] != 4) return false;
	unsigned int data[2];
...
virtual bool ackState(HerkulexPacket& ack, State& state, Status& status) const 
{
	// read 54 throw 60, at addr 58 
	if (ack.servo_id != hw_id) return false;
	if (ack.command != HerkulexPacket::ACK_RAM_READ) return false;
	if (ack.data.size() != 18) return false;
	if (ack.data[0] != 58 || data[1] != 14) return false;
	unsigned int data[7];
	if (!ackRead_impl(ack, 54, data, status)) return false;
	state.pos = convertPosRawToRad(data[0]);
	state.vel = convertVelRawToRad(data[1]);
	state.pwm = data[2] / 1023;
